...
$$B_k = \\frac{1}{\\pi} \\int\_{-\\pi}^{\\pi}f(x)sin(kx)dx = \\frac{1}{||\\sin (kx)||^2} \<f(x), \\sin (kx)>$$
...
$$\\mathcal{L}{x(t)} = X(S) = \\int\_{-\\infty}^{\\infty} x(t) e^{-S t} dt, S = \\sigma + j \\omega$$
...
$$X(S) = \\int\_{-\\infty}^{\\infty} x(t) e^{-S t} dt, S = \\sigma + j \\omega$$

$$X(S) = \\int\_{-\\infty}^{\\infty} x(t) e^{-\\sigma t} e^{-j \\omega t} dt$$
