...
- A **Markov Reward Process (MRP)** is an extension of a [Markov Chain](2_concepts/markov%20chain.md) where the addition of rewards for each state
...
- For an MRP, the [value function](2_concepts/value%20functions.md), $V(s)$, represents the expected return (cumulative discounted rewards) starting from state $s$
- [Bellman equation](2_concepts/bellman%20equations.md) for the value function is:
$$
