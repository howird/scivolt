...
- DDPG is the spiritual successor to [DQN](2_concepts/deep-q-networks.md), but adds an explicit actor for control which is essential for continuous action spaces
...
- For the critic parameter update, $\theta^Q$ is updated with the $\text{MSE}$ loss of $Q$'s [TD Error](2_concepts/temporal-difference-policy-evaluation.md) with the state and actions as inputs and the gradient update done with respect to the parameters
...
- As in [DQN](2_concepts/deep-q-networks.md) action repeats were used:
