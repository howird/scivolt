...
> [!info] Background
> - From [Value Functions](value-functions.md), we have the following formulations for $V^*(s)$ and $\pi^*(s)$:
>
> $$
> \begin{aligned}
> V^*(s) &= \max_\pi V(s) \\
...
> $$
> 
> - similarly to how we were able to split up the equation for the [return function](2_concepts/return-function.md) $G$ to: $G_t = R_{t+1} + \gamma G_{t+1}$, we can do the same for $V(s)$:
> $$
...
- These are examples of [dynamic programming rl algorithms](2_concepts/dynamic-programming-rl.md)
