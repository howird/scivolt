...
- The __Fundamental Matrix__, $F$, enforces the [epipolar constraint](2_concepts/epipolar%20geometry.md) to relate the pixel coordinates on two images $\tilde{\mathbf{x}}_l$ and $\tilde{\mathbf{x}}_r$
...
- Recall [camera intrinsic parameters](2_concepts/camera%20calibration.md), $K$, where:
$$
K = \begin{bmatrix}
f_x & s & c_x \\
0 & f_y & c_y \\
...
- the [Essential Matrix](2_concepts/epipolar%20geometry.md), $E$, assumes that both the cameras have the same intrinsic parameters $K=I$, obviously this is not always the case
