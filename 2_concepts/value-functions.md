...
> [!background]
> - Recall  the [return function](2_concepts/return-function.md), $G$, is not especially meaningful or tangible - it is more of a theoretical formulation - we cannot know the sum of future rewards without taking the actions $\implies$ thus we would need to know the future to calculate it
...
- given this mathematical formulation, we must determine $V(s)$ using either the [value iteration or policy iteration](2_concepts/policy-value-iteration.md) algorithms (in the tabular setting)
