...
- On each of an infinite sequence of time steps $t=1, 2, 3, ...$
    - at each time step, $t$, chose action $A_t \in A$ where $k=|A|$
    - and receive a reward $R_t\in \mathbb R$ from the environment
- The reward depends only on the action taken and is [i.i.d](2_concepts/individually%20identically%20distributed.md) 
